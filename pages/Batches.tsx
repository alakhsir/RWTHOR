          ...
          {filteredBatches.map(batch => (
            <div key={batch.id} className="bg-surface border border-border rounded-xl overflow-hidden flex flex-col hover:border-gray-600 transition-all group">
              <div className="relative h-48">
                ...
                {/* New Content Count Badge - Top Right */}
                {batch.newContentCount && batch.newContentCount > 0 && (
                  <div className="absolute top-3 right-3 bg-gradient-to-br from-purple-600 to-purple-800 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg border border-purple-400/30 backdrop-blur-sm flex items-center gap-1.5">
                    <span className="w-2 h-2 bg-white rounded-full animate-pulse"></span>
                    {batch.newContentCount} New
                  </div>
                )}
